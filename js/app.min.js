...
app.controller("fbMessageCleanerCtrl", function ($scope, $http) {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            if (tabs[0].url.search(VERIFY_FB_MESSAGE_URL) >= 0)
                chrome.tabs.sendMessage(tabs[0].id, { req: SELECT_AND_DELETE }, function (response) {
                    ...
                    else { $scope.updateContentScreenView(DELETE_SCREEN); $scope.updateContentScreenView(DELETE_SCREEN) }
                });
            else alert("Please open a page on Instagram and log in first.")
        })
    }; $scope.initSendMessage = function initSendMessage() {
        $scope.myCMShowMessage(); document.getElementById("ChoixAnnonces").style.display = "none"; document.getElementById("ChoixMessage").style.display = "none"; document.getElementById("messageToSend").style.display =
            "block"; document.getElementById("EnvoiMessage").style.display = "block";
        document.getElementById("SelectCategory").style.display = "block"; document.getElementById("HelpCat").style.display = "block";
        document.getElementById("Reinit").style.display = "block"; document.getElementById("TempsAttente").style.display = "block";
        document.getElementById("Explication").style.display = "block"; chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, { req: SEND_MESSAGE }, function (response) {
                if (response ==
                    true) window.close(); else { $scope.updateContentScreenView(DELETE_SCREEN); $scope.updateContentScreenView(DELETE_SCREEN) }
            })
        })
    ...
    $scope.insertFbIframe = function insertFbIframe() { document.getElementById("fb_iframe").insertAdjacentHTML("beforeend", '<iframe src="https://www.facebook.com/plugins/share_button.php?href=https%3A%2F%2Fwww.facebook.com%2FFacbook-Mesage-Cleaner-988181211327356&layout=button_count&size=large&mobile_iframe=true&appId=1825954647679729&width=98&height=28" width="98" height="28" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true"></iframe>') }; $scope.updateContentScreenView =
        function updateContentScreenView(screen) { $scope.contentViewScreen = screen; if (!$scope.$$phase) $scope.$apply() };
    ...
    $scope.init = function init() {
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            if (tabs[0].url.search(VERIFY_FB_MESSAGE_URL) >=
                0) chrome.storage.local.get("categories", function (result) {
